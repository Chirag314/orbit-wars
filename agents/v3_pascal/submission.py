# Candidate 054: 053 stricter deadline reserve; starts pruning earlier and harder to eliminate seat3 core_pool DeadlineExceeded while preserving 048 full-quality path with healthy budget.
# Hybrid generated 2026-05-02 from Docker001 selective FFA graft. Variant orbitbot_hybrid_20260502_008.
import math
import time
...
LATE_REMAINING_TURNS = 62
VERY_LATE_REMAINING_TURNS = 21
...
STATIC_HOSTILE_VALUE_MULT = 1.71
ROTATING_OPENING_VALUE_MULT = 0.9
HOSTILE_TARGET_VALUE_MULT = 1.71
OPENING_HOSTILE_TARGET_VALUE_MULT = 1.28
...
REINFORCE_VALUE_MULT = 1.32
CRASH_EXPLOIT_VALUE_MULT = 1.18
FINISHING_HOSTILE_VALUE_MULT = 1.29
BEHIND_ROTATING_NEUTRAL_VALUE_MULT = 0.92
FFA_OPPORTUNISM_ENABLED = True
...
SEAT_REPAIR_ENABLED = True
SEAT_REPAIR_PLAYERS = {0, 1, 3}
SEAT_REPAIR_START_STEP = 30
SEAT_REPAIR_HOSTILE_VALUE_MULT = 1.06
SEAT_REPAIR_LEADER_VALUE_MULT = 1.14
SEAT_REPAIR_LEADER_SCORE_MULT = 1.08
SEAT_REPAIR_HOSTILE_SEND_BONUS = 1
SEAT_REPAIR_NEUTRAL_SCORE_MULT = 0.98
...
SWARM_SCORE_MULT = 1.06  # v24 REVERTED: 1.12 caused Kaggle evaluation crash (runtime error in Orbit Wars game sim); revert to 1.06
CRASH_EXPLOIT_SCORE_MULT = 1.05
EXPOSED_PLANET_SCORE_MULT = 1.18
BLOOD_IN_WATER_SCORE_MULT = 1.12
WEAKEST_ENEMY_SCORE_MULT = 1.08

FOLLOWUP_MIN_SHIPS = 11
...
REINFORCE_MAX_SOURCE_FRACTION = 0.66
REINFORCE_MIN_FUTURE_TURNS = 40
REINFORCE_HOLD_LOOKAHEAD = 20
REINFORCE_COST_TURN_WEIGHT = 0.28
...
MULTI_SOURCE_TOP_K = 7
MULTI_SOURCE_ETA_TOLERANCE = 3  # v25: 2->3 loosen timing window for multi-source launches; safer direction than tight=0
...
LATE_IMMEDIATE_SHIP_VALUE = 0.72

WEAK_ENEMY_THRESHOLD = 45
ELIMINATION_BONUS = 28
...
SOFT_ACT_DEADLINE = 0.74
ACT_TIMEOUT_FRACTION = 0.72
ACT_TIMEOUT_RESERVE = 0.06
MIN_ACT_BUDGET = 0.18
HEAVY_PHASE_MIN_TIME = 0.20
OPTIONAL_PHASE_MIN_TIME = 0.11
HEAVY_ROUTE_PLANET_LIMIT = 32
RUNTIME_PRUNE_ENABLED = True
RUNTIME_ATTACK_SOURCE_TOP_K = 10
RUNTIME_ATTACK_TARGET_TOP_K = 18
RUNTIME_FOLLOWUP_TARGET_TOP_K = 12
RUNTIME_DOOMED_TARGET_TOP_K = 12
RUNTIME_PRUNE_MIN_TIME = 0.75
...
def seat_repair_active(world):
    return (
        SEAT_REPAIR_ENABLED
        and world.is_four_player
        and world.player in SEAT_REPAIR_PLAYERS
        and world.step >= SEAT_REPAIR_START_STEP
    )
...
def target_value(target, arrival_turns, mission, world, modes, policy):
    ...
    if seat_repair_active(world) and target.owner not in (-1, world.player):
        value *= SEAT_REPAIR_HOSTILE_VALUE_MULT
        if target.owner == world.leader_enemy:
            value *= SEAT_REPAIR_LEADER_VALUE_MULT
            value += target.production * LEADER_DENIAL_PRODUCTION_BONUS
...
def preferred_send(target, base_needed, arrival_turns, src_available, world, modes, policy):
    ...
    if seat_repair_active(world) and target.owner not in (-1, world.player):
        margin += SEAT_REPAIR_HOSTILE_SEND_BONUS
...
def apply_score_modifiers(base_score, target, mission, world):
    ...
    if seat_repair_active(world):
        if target.owner == world.leader_enemy:
            score *= SEAT_REPAIR_LEADER_SCORE_MULT
        elif target.owner == -1 and not world.is_static(target.id):
            score *= SEAT_REPAIR_NEUTRAL_SCORE_MULT
    if (
...
def plan_moves(world, deadline=None):
    ...
    def prune_limit(base_limit):
        if not RUNTIME_PRUNE_ENABLED:
            return None
        # When the budget is already thin, avoid spending the remaining turn
        # on low-yield route probes. With no deadline/full budget, keep 048's
        # behavior exactly so tactical quality is preserved in normal turns.
        if deadline is None or time_left() > RUNTIME_PRUNE_MIN_TIME:
            return None
        return base_limit

    def ranked_attack_sources():
        sources = [src for src in world.my_planets if source_attack_left(src.id) > 0]
        limit = prune_limit(RUNTIME_ATTACK_SOURCE_TOP_K)
        if limit is None or len(sources) <= limit:
            return sources
        center_bias = 0.015 if world.is_four_player else 0.0
        return sorted(
            sources,
            key=lambda src: -(
                source_attack_left(src.id)
                + src.production * 4.0
                - center_bias * dist(src.x, src.y, CENTER_X, CENTER_Y)
            ),
        )[:limit]

    def ranked_targets_for_source(src, base_limit):
        targets = [target for target in world.planets if target.id != src.id and target.owner != world.player]
        limit = prune_limit(base_limit)
        if limit is None or len(targets) <= limit:
            return targets
        def target_key(target):
            hostile_bonus = 18.0 if target.owner not in (-1, world.player) else 0.0
            comet_penalty = 6.0 if target.id in world.comet_ids and target.production <= LOW_VALUE_COMET_PRODUCTION else 0.0
            return -(
                target.production * 10.0
                + hostile_bonus
                - int(target.ships) * 0.35
                - dist(src.x, src.y, target.x, target.y) * 0.12
                - comet_penalty
            )
        return sorted(targets, key=target_key)[:limit]
    ...
    # from a real route.
    for src in ranked_attack_sources():
        if expired():
        ...
        for target in ranked_targets_for_source(src, RUNTIME_ATTACK_TARGET_TOP_K):
            if expired():
                return finalize_moves()
    ...
    if not world.is_very_late and allow_optional_phase():
        for src in ranked_attack_sources():
            ...
            best = None
            for target in ranked_targets_for_source(src, RUNTIME_FOLLOWUP_TARGET_TOP_K):
                if expired():
                    return finalize_moves()
                if target.id == src.id or target.owner == world.player:
                    continue
    ...
    if live_doomed:
        ...
        for planet in world.my_planets:
            ...
            best_capture = None
            for target in ranked_targets_for_source(planet, RUNTIME_DOOMED_TARGET_TOP_K):
                if expired():
                    return finalize_moves()
    ...
    if (
        ...
        if safe_fronts:
            ...
            # Production-dominance state-conditioned rear send ratio
            base_ratio = REAR_SEND_RATIO_FOUR_PLAYER if world.is_four_player else REAR_SEND_RATIO_TWO_PLAYER
            send_ratio = base_ratio

            # Compute production dominance: my best planet prod vs nearest enemy best planet prod
            my_best_prod = max((p.production for p in world.my_planets), default=0)
            if my_best_prod > 0 and world.enemy_planets:
                enemy_prods = [p.production for p in world.enemy_planets]
                nearest_enemy_prod = max(enemy_prods)
                prod_ratio = my_best_prod / nearest_enemy_prod
            else:
                prod_ratio = 1.0
...
def agent(obs, config=None):
    ...
    if not world.my_planets:
        return []
    try:
        act_timeout = float(_read(config, "actTimeout", 1.0)) if config is not None else 1.0
    except (TypeError, ValueError):
        act_timeout = 1.0
    act_timeout = max(0.01, act_timeout)
    soft_budget = min(SOFT_ACT_DEADLINE, max(MIN_ACT_BUDGET, act_timeout * ACT_TIMEOUT_FRACTION))
    soft_budget = min(soft_budget, max(0.01, act_timeout - ACT_TIMEOUT_RESERVE))
