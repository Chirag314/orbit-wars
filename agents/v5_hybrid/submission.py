...
def search_step_action(world, max_per_source=3, max_actions_to_eval=10,
    ...
    # Idea 2 — MC re-scoring: re-rank top candidates using rollout simulations
    scored = mc_rescore_actions(world, scored)
...
# Exploit fleet-speed scaling: fat fleets arrive faster AND leave
# surplus ships that chain-capture nearby planets.
# ═══════════════════════════════════════════════════════════════════
MOMENTUM_ENABLED        = True
MOMENTUM_CLUSTER_RADIUS = 20.0   # planets within this radius form a cluster
MOMENTUM_MIN_CLUSTER    = 2      # minimum planets in cluster
MOMENTUM_MIN_PROD       = 3      # minimum cluster production to bother
MOMENTUM_SPEED_GAIN     = 0.8    # fleet_speed(fat) - fleet_speed(min) must exceed this
MOMENTUM_MAX_SRC_DIST   = 35.0   # source planet must be within this distance
MOMENTUM_KEEP_SHIPS     = 10     # always keep this many on source after dispatch

def _momentum_cluster(world, seed):
    """All non-friendly planets within MOMENTUM_CLUSTER_RADIUS of seed."""
    cluster, total_prod, total_garrison = [], 0, 0
    for p in world.planets:
        if p.owner == world.player:
            continue
        if dist(seed.x, seed.y, p.x, p.y) > MOMENTUM_CLUSTER_RADIUS:
            continue
        cluster.append(p)
        total_prod    += int(p.production)
        total_garrison += int(p.ships)
    return cluster, total_prod, total_garrison

def _fat_fleet_size(world, src, tgt, avail):
    """
    Find the fleet size N that maximises ships leftover after capturing tgt.
    Speed advantage: larger N → faster fleet → defender has less production time.
    Returns (angle, N, turns, leftover) or None.
    """
    best = None
    for frac in (1.0, 0.75, 0.5):
        n = max(MIN_DISPATCH_SHIPS, int(avail * frac))
        if n > avail:
            n = avail
        a_info = aim_at_target(src, tgt, n, world.initial_by_id, world.ang_vel, world)
        if a_info is None:
            continue
        angle, turns = a_info
        _, eff_garrison = effective_garrison_at_arrival(tgt, turns, world)
        needed = int(eff_garrison) + 1
        if n < needed:
            continue
        leftover = n - needed
        speed    = fleet_speed(n)
        # compare with minimum viable fleet speed
        min_a = aim_at_target(src, tgt, needed, world.initial_by_id, world.ang_vel, world)
        min_speed = fleet_speed(needed) if min_a else 1.0
        speed_gain = speed - min_speed
        if best is None or leftover > best[3]:
            best = (angle, n, turns, leftover, speed_gain)
    return best

def handle_momentum_strike(world, moves, spent, target_locked):
    """
    Idea 1: find the best (source, cluster) pair where sending a fat fleet
    gives a meaningful speed bonus AND leaves enough ships to chain-capture
    the rest of the cluster.  Execute at most one momentum strike per turn.
    """
    ...
    best_score = -1e9
    best_src = best_tgt = best_angle = best_n = None

    for tgt in world.enemy_planets + world.neutral_planets:
        if tgt.id in target_locked:
            continue
        cluster, c_prod, c_garrison = _momentum_cluster(world, tgt)
        if c_prod < MOMENTUM_MIN_PROD or len(cluster) < MOMENTUM_MIN_CLUSTER:
            continue

        for src in world.my_planets:
            d = dist(src.x, src.y, tgt.x, tgt.y)
            if d > MOMENTUM_MAX_SRC_DIST:
                continue
            avail = int(src.ships) - spent.get(src.id, 0) - MOMENTUM_KEEP_SHIPS
            if avail < MIN_DISPATCH_SHIPS:
                continue

            result = _fat_fleet_size(world, src, tgt, avail)
            if result is None:
                continue
            angle, n, turns, leftover, speed_gain = result

            if speed_gain < MOMENTUM_SPEED_GAIN:
                continue   # not a meaningful speed boost

            # Score: leftover coverage of remaining cluster + speed gain + production
            remaining = max(1, c_garrison - int(tgt.ships))
            score = (leftover / remaining) * speed_gain + c_prod * 1.5
            if score > best_score:
                best_score = score
    ...
    if best_src is not None:
        moves.append([best_src.id, best_angle, best_n])
        spent[best_src.id] = spent.get(best_src.id, 0) + best_n
        target_locked.add(best_tgt.id)

# ═══════════════════════════════════════════════════════════════════
# IDEA 2 — Monte Carlo rollout evaluator
# Re-score the top search candidates using short forward simulations
# rather than the linear heuristic.  Replaces melis_evaluate for the
# final ranking step inside search_step_action.
# ═══════════════════════════════════════════════════════════════════
MC_EVAL_ENABLED  = True
MC_N_ROLLOUTS    = 2      # rollouts per candidate
MC_HORIZON       = 18     # turns per rollout
MC_EMIT_FRAC     = 0.45   # opponent greedy emit fraction in rollout
MC_TOP_K_RESCORE = 5      # only re-score top-K pre-ranked candidates

def _mc_sim_state(world):
    """Lightweight state dict for rollout simulation."""
    owners, ships_m, prod_m, xy = {}, {}, {}, {}
    for p in world.planets:
        owners[p.id]  = int(p.owner)
        ships_m[p.id] = float(p.ships)
        prod_m[p.id]  = int(p.production)
        xy[p.id]      = (float(p.x), float(p.y))
    fleets = [[int(f.owner), float(f.x), float(f.y),
               float(f.angle), int(f.ships)] for f in world.fleets]
    return {"owners": owners, "ships": ships_m, "prod": prod_m,
            "xy": xy, "fleets": fleets, "pids": list(owners.keys())}

def _mc_inject(state, src_id, angle, n_ships):
    if state["ships"].get(src_id, 0) < n_ships:
        return
    state["ships"][src_id] -= n_ships
    sx, sy = state["xy"][src_id]
    state["fleets"].append([state["owners"][src_id], sx, sy, angle, n_ships])

def _mc_step(state):
    arrivals = defaultdict(list)
    surviving = []
    for pid in state["pids"]:
        if state["owners"][pid] != -1:
            state["ships"][pid] += state["prod"][pid]
    for fl in state["fleets"]:
        o, fx, fy, ang, fs = fl
        if fs <= 0:
            continue
        ...
        fl[1], fl[2] = nx, ny
        # crude sun check
        mid_d = math.hypot((fx + nx) / 2 - CENTER_X, (fy + ny) / 2 - CENTER_Y)
        if mid_d < SUN_R + SUN_SAFETY:
            continue
        hit = None
        for pid in state["pids"]:
            px, py = state["xy"][pid]
            if (nx - px) ** 2 + (ny - py) ** 2 < 9.0:
                hit = pid
                break
        if hit is not None:
    ...
    for pid, arr in arrivals.items():
        ...
        for o, s in arr:
            by_o[o] += s
        ranked = sorted(by_o.items(), key=lambda kv: -kv[1])
        top_o, top_s = ranked[0]
        sec_s = ranked[1][1] if len(ranked) > 1 else 0
        surv  = top_s - sec_s
        if surv <= 0:
            continue
        cur = state["owners"][pid]
        if cur == top_o:
            state["ships"][pid] += surv
        else:
            state["ships"][pid] -= surv
            if state["ships"][pid] < 0:
                state["owners"][pid] = top_o
                state["ships"][pid]  = -state["ships"][pid]

def _mc_greedy(state, player):
    """Each owned planet emits MC_EMIT_FRAC ships toward nearest non-friendly."""
    for pid in state["pids"]:
        if state["owners"][pid] != player:
            continue
        s = state["ships"][pid]
        send = int(s * MC_EMIT_FRAC)
        if send < 5:
            continue
        sx, sy = state["xy"][pid]
        best_d, best_ang = 1e9, None
        for opid in state["pids"]:
            if state["owners"][opid] == player:
                continue
            ox, oy = state["xy"][opid]
        ...
        if best_ang is not None:
            state["ships"][pid] -= send
            state["fleets"].append([player, sx, sy, best_ang, send])

def _mc_score_state(state, player):
    n_s = n_p = n_r = 0
    ...
    for fl in state["fleets"]:
        if fl[0] == player:
            ...
            opp[fl[0]][0] += fl[4]
    if not opp:
        return n_s + 5 * n_p + 8 * n_r
    best_e = max(v[0] + 5 * v[1] + 8 * v[2] for v in opp.values())
    return (n_s + 5 * n_p + 8 * n_r) - best_e

def mc_evaluate(world, action, n_rollouts=MC_N_ROLLOUTS, horizon=MC_HORIZON):
    """Score an action using Monte Carlo rollouts."""
    src_id = action.get("source_id")
    angle  = action.get("angle")
    ships  = action.get("ships")
    base   = _mc_sim_state(world)
    total  = 0.0
    for _ in range(n_rollouts):
        state = copy.deepcopy(base)
        if src_id is not None:
            _mc_inject(state, src_id, angle, ships)
        for t in range(horizon):
            if t % 3 == 0:
                for o in set(state["owners"].values()):
                    if o not in (-1, world.player):
                        _mc_greedy(state, o)
            _mc_step(state)
        total += _mc_score_state(state, world.player)
    return total / max(1, n_rollouts)

def mc_rescore_actions(world, actions):
    """
    Idea 2: re-rank the top MC_TOP_K_RESCORE actions using MC rollouts.
    Returns the re-sorted list (full list, MC-scored portion first).
    """
    if not MC_EVAL_ENABLED or not actions:
        return actions
    top = actions[:MC_TOP_K_RESCORE]
    rest = actions[MC_TOP_K_RESCORE:]
    # baseline (no action)
    base_score = mc_evaluate(world, {})
    rescored = []
    for act in top:
        mc = mc_evaluate(world, act)
        gain = mc - base_score
        rescored.append((gain, act))
    rescored.sort(key=lambda x: -x[0])
    return [a for _, a in rescored] + rest

# ═══════════════════════════════════════════════════════════════════
# IDEA 3 — 2P Minimax (alpha-beta, depth 2)
# In 1v1 games during the combat phase, use adversarial search to
# find moves the opponent cannot profitably counter.
# ═══════════════════════════════════════════════════════════════════
MM_ENABLED   = True
MM_MIN_STEP  = 80     # don't minimax during early expansion
MM_DEPTH     = 2
MM_BRANCH    = 5      # top-k actions per player per node
MM_TIME_FRAC = 0.45   # fraction of remaining budget to spend on minimax

def _mm_actions(world, state, player, k=MM_BRANCH):
    """Top-k candidate actions for minimax node."""
    acts = [(None, None, None)]  # pass
    for pid in state["pids"]:
        if state["owners"][pid] != player:
            continue
        avail = int(state["ships"][pid]) - MIN_DISPATCH_SHIPS
        if avail < MIN_DISPATCH_SHIPS:
            continue
        sx, sy = state["xy"][pid]
        for opid in state["pids"]:
            ...
            ox, oy = state["xy"][opid]
            ang = math.atan2(oy - sy, ox - sx)
            mid_d = math.hypot((sx + ox) / 2 - CENTER_X, (sy + oy) / 2 - CENTER_Y)
            if mid_d < SUN_R + SUN_SAFETY:
                continue
            prod_val = state["prod"][opid]
            d        = math.hypot(ox - sx, oy - sy)
            score    = prod_val * 4 - d * 0.1
            acts.append((pid, ang, avail, score))
    acts_ranked = sorted([a for a in acts if a[0] is not None],
                         key=lambda x: -x[3])[:k]
    return [(None, None, None)] + [(pid, ang, av) for pid, ang, av, _ in acts_ranked]

def _alphabeta(state, world, depth, alpha, beta, maximising, deadline):
    if time.perf_counter() > deadline or depth == 0:
        return _mc_score_state(state, world.player)
    player = world.player if maximising else _mm_enemy(world)
    actions = _mm_actions(world, state, player)
    if maximising:
        val = -1e9
        for pid, ang, av in actions:
            s2 = copy.deepcopy(state)
            if pid is not None:
                _mc_inject(s2, pid, ang, av)
            _mc_step(s2)
            val = max(val, _alphabeta(s2, world, depth - 1,
        ...
        for pid, ang, av in actions:
            ...
            if pid is not None:
                _mc_inject(s2, pid, ang, av)
            _mc_step(s2)
            val = min(val, _alphabeta(s2, world, depth - 1,
...
def handle_minimax_2p(world, moves, spent, target_locked, deadline):
    """
    Idea 3: for 2P combat phase, find the best single offensive action
    via alpha-beta and prepend it to moves if it scores better than pass.
    """
    if not MM_ENABLED or not world.is_2p:
        return
    if world.step < MM_MIN_STEP:
        return
    mm_deadline = time.perf_counter() + (deadline - time.perf_counter()) * MM_TIME_FRAC
    if mm_deadline < time.perf_counter() + 0.1:
        return

    base_state = _mc_sim_state(world)
    best_val   = _mc_score_state(base_state, world.player)  # pass baseline
    best_move  = None

    for src in sorted(world.my_planets, key=lambda p: -int(p.ships)):
        avail = int(src.ships) - spent.get(src.id, 0) - MIN_DISPATCH_SHIPS
        if avail < MIN_DISPATCH_SHIPS:
            continue
        for tgt in world.enemy_planets + world.neutral_planets:
            if tgt.id in target_locked:
            ...
            if time.perf_counter() > mm_deadline:
                break
            a_info = aim_at_target(src, tgt, avail,
                                   world.initial_by_id, world.ang_vel, world)
            if a_info is None:
                continue
            angle, _ = a_info
            s2 = copy.deepcopy(base_state)
            _mc_inject(s2, src.id, angle, avail)
            _mc_step(s2)
            val = _alphabeta(s2, world, MM_DEPTH - 1,
    ...
    if best_move:
        ...
        spent[best_move[0]] = spent.get(best_move[0], 0) + best_move[2]
        # find which target this is aimed at and lock it
        for tgt in world.enemy_planets + world.neutral_planets:
            a_info = aim_at_target(
                world.planet_by_id[best_move[0]], tgt, best_move[2],
                world.initial_by_id, world.ang_vel, world)
            if a_info and abs(a_info[0] - best_move[1]) < 0.05:
                target_locked.add(tgt.id)
                break
...
def plan_moves(world, deadline=None):
    ...
    # Idea 1 — Momentum Strike (after hammer so it doesn't steal hammer targets)
    if not _over_budget():
        handle_momentum_strike(world, moves, spent, target_locked)

    # Idea 3 — 2P Minimax (combat phase only, after expansion)
    if not _over_budget() and deadline is not None:
        handle_minimax_2p(world, moves, spent, target_locked, deadline)
