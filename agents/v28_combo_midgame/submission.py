...
class ProducerLiteConfig:
    ...
    max_offensive_targets: int = 14         # enemy/neutral proximity targets
    max_defensive_targets: int = 4
    # --- scoring / greedy ------------------------------------------------
    max_waves_per_turn: int = 10
...
def run_turn(obs_tensors: dict, *, config: ProducerLiteConfig, player_count: int, memory) -> dict:
    ...
    """
    # Late-game: lower ROI threshold for more aggressive play in final 50 turns
    _step = int(obs_tensors["step"].item()) if hasattr(obs_tensors["step"], "item") else int(obs_tensors["step"])
    if _step > 200:
        config = dataclasses.replace(config, roi_threshold=1.2)
    if _step > 300:
        config = dataclasses.replace(config, roi_threshold=0.9)
    if _step > 400:
        config = dataclasses.replace(config, roi_threshold=0.7)
