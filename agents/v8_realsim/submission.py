...
# ── Real-simulator evaluation ────────────────────────────────────────────────
# Use the game's exact swept-pair collision detection instead of our
# approximate forward_project.  Only imported once at module load.
try:
    import kaggle_environments.envs.orbit_wars.orbit_wars as _ow
    _SWEPT   = _ow.swept_pair_hit
    _PT_SEG  = _ow.point_to_segment_distance
    _OW_CENTER   = _ow.CENTER
    _OW_SUN_R    = _ow.SUN_RADIUS
    _OW_BOARD    = _ow.BOARD_SIZE
    _OW_ROT_LIM  = _ow.ROTATION_RADIUS_LIMIT
    _REAL_SIM_OK = True
except Exception:
    _REAL_SIM_OK = False

REAL_SIM_ENABLED    = True   # master switch
REAL_SIM_HORIZON    = 22     # turns per rollout
REAL_SIM_ROLLOUTS   = 2      # rollouts per candidate
REAL_SIM_TOP_K      = 5      # re-score only top-K pre-ranked candidates
REAL_SIM_EMIT_FRAC  = 0.40   # greedy emit fraction for simulated opponents

def _to_sim_state(world):
    """Convert World to mutable state dict for real_step."""
    planets = [[p.id, p.owner, float(p.x), float(p.y),
                float(p.radius), float(p.ships), int(p.production)]
               for p in world.planets]
    fleets  = [[f.id, f.owner, float(f.x), float(f.y),
                float(f.angle), f.from_planet_id, int(f.ships)]
               for f in world.fleets]
    init_by_id = {pid: [p.id, p.owner, float(p.x), float(p.y),
                         float(p.radius), float(p.ships), int(p.production)]
                  for pid, p in world.initial_by_id.items()}
    nfid = max((f.id for f in world.fleets), default=0) + 1
    return {
        "planets":      planets,
        "fleets":       fleets,
        "init_by_id":   init_by_id,
        "ang_vel":      float(world.ang_vel),
        "step":         int(world.step) + 1,
        "next_fid":     nfid,
    }

def _real_step(st, actions_by_player):
    """Exact single-step using ow.swept_pair_hit. Mutates st in place."""
    planets = st["planets"]
    fleets  = st["fleets"]
    ibi     = st["init_by_id"]
    ang_vel = st["ang_vel"]
    ...
    # 0. Launch
    for pid, moves in actions_by_player.items():
        for from_id, angle, ships in (moves or []):
            ships = int(ships)
            src = next((p for p in planets if p[0] == from_id), None)
            if src and src[1] == pid and src[5] >= ships > 0:
                src[5] -= ships
                sx = src[2] + math.cos(angle) * (src[4] + 0.1)
                sy = src[3] + math.sin(angle) * (src[4] + 0.1)
                fleets.append([nfid, pid, sx, sy, angle, from_id, ships])
                nfid += 1
    ...
    # 2. Planet paths (orbital motion)
    ppaths = {}
    for p in planets:
        old = (p[2], p[3])
        ip = ibi.get(p[0])
        if ip is not None:
            dx, dy = ip[2] - ctr, ip[3] - ctr
            r = math.sqrt(dx*dx + dy*dy)
            if r + p[4] < _OW_ROT_LIM:
                ia = math.atan2(dy, dx)
                na = ia + ang_vel * step
                new = (ctr + r * math.cos(na), ctr + r * math.sin(na))
            else:
                new = old
        else:
            new = old
    ...
    for fl in fleets:
        ...
        spd = min(spd, 6.0)
        ox, oy = fl[2], fl[3]
        fl[2] += math.cos(ang) * spd
        fl[3] += math.sin(ang) * spd
        nx, ny = fl[2], fl[3]
        old_pos, new_pos = (ox, oy), (nx, ny)
        hit = False
        for p in planets:
            po, pn = ppaths[p[0]]
            if _SWEPT(old_pos, new_pos, po, pn, p[4]):
                combat[p[0]].append(fl)
                dead.append(fl)
                hit = True
                break
        if hit:
            continue
        if not (0 <= nx <= _OW_BOARD and 0 <= ny <= _OW_BOARD):
            dead.append(fl); continue
        if _PT_SEG((ctr, ctr), old_pos, new_pos) < _OW_SUN_R:
            dead.append(fl); continue

    # 4. Apply planet orbital positions — unpack new position from (old, new)
    for p in planets:
        p[2], p[3] = ppaths[p[0]][1]

    st["fleets"] = [f for f in fleets if f not in dead]

    # 5. Combat
    dead_set = set(id(f) for f in dead)
    for pid, arr in combat.items():
        planet = next((p for p in planets if p[0] == pid), None)
        if planet is None or not arr:
            continue
        by_o = defaultdict(int)
        for f in arr:
            by_o[f[1]] += f[6]
        ranked = sorted(by_o.items(), key=lambda x: -x[1])
        top_o, top_s = ranked[0]
        sec_s = ranked[1][1] if len(ranked) > 1 else 0
        surv  = top_s - sec_s if top_s != sec_s else 0
        if surv > 0:
            if planet[1] == top_o:
                planet[5] += surv
            else:
...
def _greedy_emit(st, player):
    """Each owned planet fires REAL_SIM_EMIT_FRAC ships at nearest non-friendly."""
    ctr = _OW_CENTER
    planets = st["planets"]
    for p in planets:
        if p[1] != player:
            continue
        send = int(p[5] * REAL_SIM_EMIT_FRAC)
        if send < 5:
            continue
        best_d, best_a = 1e9, None
        for t in planets:
            if t[1] == player:
                continue
            dx, dy = t[2] - p[2], t[3] - p[3]
            d = math.sqrt(dx*dx + dy*dy)
            if d < best_d:
                ang = math.atan2(dy, dx)
                mid_x = (p[2] + t[2]) / 2
                mid_y = (p[3] + t[3]) / 2
                if math.sqrt((mid_x-ctr)**2 + (mid_y-ctr)**2) > _OW_SUN_R + 1.5:
                    best_d, best_a = d, ang
        if best_a is not None:
            p[5] -= send
            fid = st["next_fid"]
            st["fleets"].append([fid, player, p[2], p[3], best_a, p[0], send])
            st["next_fid"] += 1

def _sim_score(st, player, n_players):
    """Final position score: advantage over best opponent."""
    my_s = my_p = my_r = 0
    opp  = defaultdict(lambda: [0, 0, 0])
    for p in st["planets"]:
        o = p[1]
        if o == player:
            my_s += p[5]; my_p += 1; my_r += p[6]
        elif o != -1:
            opp[o][0] += p[5]; opp[o][1] += 1; opp[o][2] += p[6]
    for f in st["fleets"]:
        if f[1] == player:
            my_s += f[6]
        elif f[1] != -1:
    ...
    return (my_s + 5*my_p + 8*my_r) - best_e

def real_evaluate(world, action, n_rollouts=REAL_SIM_ROLLOUTS,
                  horizon=REAL_SIM_HORIZON):
    """
    Score an action using exact game-engine rollouts.
    action: dict with source_id, angle, ships (or None for baseline).
    """
    if not _REAL_SIM_OK or not REAL_SIM_ENABLED:
        return melis_evaluate(world, our_step_action=action)

    n_players = 2 if world.is_2p else 4
    base_state = _to_sim_state(world)
    total = 0.0
    ...
    for _ in range(n_rollouts):
        ...
        if action is not None:
            ...
            if src_id is not None:
                my_acts = [[src_id, angle, int(ships)]]

        for t in range(horizon):
            acts = {world.player: my_acts if t == 0 else []}
            # Opponents: greedy every 3 turns
            if t % 3 == 0:
                for o in set(p[1] for p in st["planets"]) - {-1, world.player}:
                    _greedy_emit(st, o)
            _real_step(st, acts)
        total += _sim_score(st, world.player, n_players)

    return total / max(1, n_rollouts)

def real_rescore(world, actions):
    """Re-rank top REAL_SIM_TOP_K actions using real_evaluate."""
    if not _REAL_SIM_OK or not REAL_SIM_ENABLED or not actions:
        return actions
    top  = actions[:REAL_SIM_TOP_K]
    rest = actions[REAL_SIM_TOP_K:]
    base = real_evaluate(world, None)   # baseline (no-action)
    rescored = []
    for act in top:
        gain = real_evaluate(world, act) - base
...
def search_step_action(world, max_per_source=3, max_actions_to_eval=10,
    ...
    # Re-rank top candidates using exact game-engine rollouts
    scored = real_rescore(world, scored)
    return scored
