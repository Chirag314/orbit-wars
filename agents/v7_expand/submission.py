...
def _detect_mode(world):
    ...
    # Fix Experiment 4: dynamic 2P mode.
    # Stay "pressure" by default (keeps aggressive far expansion).
    # Only switch to "patient" when definitively behind on production
    # AND past the expansion race (turn 60+) — the only case where
    # defensive consolidation is clearly better than pushing.
    # Add: when enemy just deployed ships, immediately go "pressure"
    # regardless of production state (counter-attack window).
    prod_ratio = world.my_prod_share
    enemy_deployed = len(_enemy_recently_launched) >= 1

    if enemy_deployed:
        mode_2p = "pressure"
    elif prod_ratio <= 0.42 and world.step >= 60:
        mode_2p = "patient"
    else:
        mode_2p = "pressure"

    _record_2p_progress(world.my_prod_share, intended_patient=(mode_2p == "patient"))
    return mode_2p
...
def _hammer_should_fire(world):
    ...
    # Counter-attack window: enemy just launched (planets depleted) → lower bar
    if world.is_2p and len(_enemy_recently_launched) >= 1:
        threshold = max(0.28, threshold - 0.10)
