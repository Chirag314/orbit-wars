...
class ProducerLiteConfig:
    ...
    max_offensive_targets: int = 16         # enemy/neutral proximity targets
    max_defensive_targets: int = 4
    # --- scoring / greedy ------------------------------------------------
    max_waves_per_turn: int = 8
    roi_threshold: float = 1.2              # fire if score > this
...
def run_turn(obs_tensors: dict, *, config: ProducerLiteConfig, player_count: int, memory) -> dict:
    ...
    if _step > 450:
        config = dataclasses.replace(config, roi_threshold=0.8)
