...
class ProducerLiteConfig:
    ...
    # Size multipliers (from Pilkwang) — evaluate several commit fractions
...
# ---------------------------------------------------------------------------
# Dynamic adjustment (Alyce Miki) — continuous ROI/waves tuning
# ---------------------------------------------------------------------------
...
def _adjust_config(
    ...
    *,
    ...
    if ratio < 1.0:
        # Behind: lower ROI to take more risks
        deficit = 1.0 - ratio
        roi_drop = 0.25 * deficit * deficit
        new_roi = max(1.10, new_roi - roi_drop)
        ...
        # Ahead: accelerate close-out — push harder when dominating
        second = float(strength.topk(min(2, strength.numel())).values[-1].item())
        lead = my / max(second, 1e-6)
        if lead > 1.5:
            # 1.5x stronger than #2: drop ROI and add waves to finish them
            push = min(0.30, (lead - 1.5) * 0.3)
            new_roi = max(1.05, new_roi - push)
            base_waves = min(8, base_waves + 1)
        if lead > 2.0 and remaining < 200:
            new_roi = max(1.00, new_roi - 0.10)
            base_waves = min(9, base_waves + 2)

    if remaining < 100 and ratio < 0.95:
        base_waves = min(8, base_waves + 1)

    config = replace(config, roi_threshold=new_roi, max_waves_per_turn=base_waves)
...
# ---------------------------------------------------------------------------
# Late-game candidate suppression (Alyce Miki)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Tiered candidates (Pilkwang) — evaluate multiple fleet sizes
# ---------------------------------------------------------------------------
...
def _tier_candidates(
    *,
    ...
    score = _suppress_late_candidates(
        ...
        cand_eta=cand_eta, step=step, player_id=pid,
    )
...
def plan_lite_waves(
    *,
    ...
    # Enemy pressure — reused for reinforcement floor and regroup
    ...
    # Tiered candidates: evaluate multiple fleet sizes (Pilkwang)
    tier_parts = [
        _tier_candidates(
            movement=movement,
...
def run_turn(obs_tensors: dict, *, config: ProducerLiteConfig, player_count: int, memory) -> dict:
    ...
    # Dynamic adjustment (Alyce): continuously tune ROI + waves based on strength
...
# ---------------------------------------------------------------------------
# Mode presets — 2P from Pilkwang params, 3P/4P from Alyce with Pilkwang tweaks
# ---------------------------------------------------------------------------
...
CONFIG_4P = replace(
    ...
    max_regroup_targets_per_source=4,
    reinforce_size_beta=1.0,
...
# ---------------------------------------------------------------------------
# Search params from JSON (Pilkwang)
# ---------------------------------------------------------------------------
...
CONFIG_3P = _ow_apply_config_overrides(CONFIG_3P, _OW_STRATEGY_PARAMS.get("config_3p"))
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        # Robust player count (Pilkwang): use player index as lower bound
